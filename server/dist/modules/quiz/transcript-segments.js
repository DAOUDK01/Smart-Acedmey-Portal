...
exports.splitTranscriptIntoSegments = splitTranscriptIntoSegments;
exports.buildStructuredTranscript = buildStructuredTranscript;
exports.buildFallbackTranscript = buildFallbackTranscript;
...
function splitTranscriptIntoSegments(transcript, durationSeconds = 600, segmentCount = 3) {
    ...
    return texts.slice(0, count).map((text, index) => ({
        ...
        text,
        timestamp: Math.max(15, Math.round(((index + 1) / (count + 1)) * safeDuration)),
        difficulty: DEFAULT_DIFFICULTIES[index % DEFAULT_DIFFICULTIES.length],
    }));
}
function buildStructuredTranscript(title, segmentTexts) {
    return segmentTexts
        .map((text, index) => `[Segment ${index + 1}]\n${text.trim()}`)
        .join("\n\n");
}
function buildFallbackTranscript(title) {
