...
export function TeacherConsole() {
  const { ready, session, isApproved } = usePortalLock("/teacher");
  ...
  const loadAll = async () => {
    const [usersData, coursesData, lecturesData, quizzesData, progressData, statsData] =
      await Promise.all([
  ...
  const handleGenerateQuiz = async () => {
    ...
    try {
      const targetLectureId =
        editingLectureId ||
        (courseId ? lectures.find((lecture) => lecture.courseId === courseId)?.id : undefined);
      ...
      await apiFetch("/api/teacher/quizzes/generate", {
        ...
        body: JSON.stringify({
          topic: lectureTitle,
          transcript,
          questionCount: lectureSegments.length || questionCount,
