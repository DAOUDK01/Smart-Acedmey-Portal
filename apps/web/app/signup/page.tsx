  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/api/admin/users`, {
          ...
          body: JSON.stringify({
            ...
            email,
            password,
            isActive: role === "ADMIN" || role === "STUDENT",
          }),
        });
      ...
      savePortalSession({
        ...
        isActive: createdUser.role === "ADMIN" || createdUser.role === "STUDENT",
        mode: "real",
      });

      if (createdUser.role === "TEACHER" || createdUser.role === "EXPERT" || createdUser.role === "GUARDIAN") {
        setStatus(`Account created! Your ${createdUser.role.toLowerCase()} profile is pending admin approval.`);
