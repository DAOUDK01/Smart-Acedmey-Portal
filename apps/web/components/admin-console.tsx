...
export function AdminConsole() {
  ...
  async function loadAll() {
    try {
      const [nextUsers, nextCourses, nextLectures, nextQuizzes, progressData] = await Promise.all([
      ...
      <div className="flex flex-col gap-6">
        ...
        {activeTab === "users" && (
          <div className="grid gap-6 lg:grid-cols-2">
            <PremiumCard eyebrow="Approvals" title="Pending Staff" description="Review and approve teacher and expert accounts.">
              <div className="mt-4 space-y-3">
                ...
                {users.filter(u => (u.role === "TEACHER" || u.role === "EXPERT") && !u.isActive).length === 0 && (
        ...
        {activeTab === "analytics" && (
          <PremiumCard eyebrow="Analytics" title="System Performance" description="Real-time platform usage and data.">
            <div className="h-80 flex items-center justify-center border-2 border-dashed border-white/5 rounded-3xl mt-4">
              <div className="text-center">
                <BarChart3 className="mx-auto h-12 w-12 text-slate-800" />
                <p className="mt-4 text-sm text-slate-600">Advanced analytics engine is starting up...</p>
              </div>
            </div>
          </PremiumCard>
        )}
