...
export function StudentConsole() {
  const { ready, session } = usePortalLock("/student");
  ...
  const [quizResult, setQuizResult] = useState<{ passed: boolean; correctAnswer: string; explanation: string } | null>(null);
  ...
  const currentLectureQuizzes = useMemo(() => {
    ...
    return quizzes
      .filter(q => q.lectureId === currentLecture.id && q.timestamp !== undefined)
  ...
  const handleTimeUpdate = () => {
    ...
    currentLectureQuizzes.forEach(quiz => {
      if (
        quiz.timestamp &&
        quiz.timestamp <= currentTime &&
        quiz.timestamp > lastCheckTime.current &&
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/teacher/quizzes/${currentQuiz.id}/attempt`, {
        method: "POST",
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-950/90 p-4 backdrop-blur-sm sm:p-8">
            <div className="relative w-full max-w-5xl overflow-hidden rounded-3xl border border-white/10 bg-ink-900 shadow-2xl">
              ...
              {currentQuiz && (
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-ink-950/95 p-4">
                  <div className="w-full max-w-2xl rounded-3xl border border-white/10 bg-ink-900 p-6 shadow-2xl">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className={cn(
                            ...
                            currentQuiz.difficulty === "easy" ? "bg-emerald-500/10 text-emerald-300" :
                              currentQuiz.difficulty === "medium" ? "bg-amber-500/10 text-amber-300" :
                                "bg-rose-500/10 text-rose-300"
                          )}>
                            {currentQuiz.difficulty}
                          </span>
                          {currentQuiz.timestamp && (
                        ...
                        {currentQuiz.topic && (
                          <p className="text-sm text-slate-400 mt-1">{currentQuiz.topic}</p>
                        )}
                      </div>
                    </div>
                    ...
                    {!quizResult ? (
                      <>
                        <h4 className="text-xl font-bold text-white mb-6">{currentQuiz.question}</h4>
                      ...
                      <>
                        <div className={cn(
                          "flex items-center gap-3 rounded-2xl p-4 mb-6",
                          quizResult.passed ? "bg-emerald-500/10 border border-emerald-500/20" : "bg-rose-500/10 border border-rose-500/20"
                        )}>
                          {quizResult.passed ? (
              ...
              <div className="aspect-video w-full bg-black">
                {getYouTubeId(currentLecture.videoUrl) ? (
                  <iframe
                    className="h-full w-full"
                    src={`https://www.youtube.com/embed/${getYouTubeId(currentLecture.videoUrl)}?autoplay=1`}
                  ...
                  >
                    <source
                      src={currentLecture.videoUrl.startsWith("http")
                        ? currentLecture.videoUrl
                        : `${API_BASE_URL}${currentLecture.videoUrl}`}
                      type="video/mp4"
                    />
              ...
              {currentLectureQuizzes.length > 0 && (
                <div className="border-t border-white/5 p-4">
                  <h5 className="text-sm font-bold text-white mb-3">Quiz Checkpoints</h5>
                  <div className="flex flex-wrap gap-2">
                    {currentLectureQuizzes.map((quiz) => (
                      <div
                        key={quiz.id}
                        className={cn(
                      ...
                      >
                        ...
                        {answeredQuizzes.has(quiz.id) && <Check size={12} />}
                      </div>
                    ))}
                  </div>
                </div>
